...
3. If all commands pass, **commit the work** (mandatory, not optional):
   ...
   c. Derive commit metadata from the run context:
       - `YYYY-MM-DD-HHmm` = date+minute parsed from `{{PLAN_FILE}}` basename (the leading date-time segment before the first `-`)
       - `scope` = mission name (e.g. `{{missionName}}`)
   d. Split changes into logical commits following the project's `AGENTS.md` commit style (imperative mood: "Add feature" not "Added feature"; reference doc paths when relevant):
