...
- [x] Decision: how `promptsDir` is exposed to `loadSubFlow`. Two viable options:
  ...
  - Skill: none
- [x] `config.js`: in the mission run branch (after `loadMission`, around line 573-578), resolve `const missionPromptsDir = mission.promptsDir ? resolve(projectRoot, mission.promptsDir) : "";` and add `missionPromptsDir` to the returned config object.
  - Skill: none
- [x] `main.js:667-671`: build `projectPromptDirs` as `[config.missionPromptsDir, resolve(config.missionsDir, "prompts")].filter(Boolean)` so the mission-level dir precedes the shared `missions/prompts/` dir. Pass to `createMissionDriverFlow`.
  - Skill: none
- [x] `flow-loader.js:306-321` `loadSubFlow`: read `this?.config?.missionPromptsDir` and build `projectPromptDirs = [missionPromptsDir, missionsDir ? resolve(missionsDir, "prompts") : ""].filter(Boolean)` (preserve the existing falsy-`missionsDir` guard so unconfigured missions keep current behavior — today's code yields `[]` when `missionsDir` is falsy). Keep the existing flow `searchDirs` (missions/flows → subflowDir → TOOL_FLOWS_DIR) unchanged.
  - Skill: none
- [x] `mission-check.mjs`: do NOT add `promptsDir` to `REQUIRED_FIELDS` (optional). Decide existence-validation: add `promptsDir` to the existence-checked list in `validateMission` (lines 73-84) so a typo'd path fails fast at mission-check — matches how `moduleDir`/`contextDir` are treated. Record the chosen behavior in the plan execution log.
  - Skill: none

Exit Criteria:

- [x] A mission with `promptsDir` set resolves prompts from that dir first, then `missions/prompts/`, then built-ins (proved by a unit test exercising `loadPrompt` / a constructed `projectPromptDirs` chain, and a `createMissionDriverFlow`+`loadSubFlow` integration test showing a mission-level prompt shadows the shared one and falls back to built-in when absent).
- [x] A mission WITHOUT `promptsDir` behaves exactly as today (shared `missions/prompts/` → built-in); existing tests stay green.
- [x] Subflow (deep-audit-loop, plan-execution) prompt loading honors the same chain — verify a subflow prompt is resolvable from the mission-level dir.
- [x] `mission-check.mjs` validates/rejects a bad `promptsDir` path per the chosen Decision (or explicitly documents why existence check is skipped).
- [x] `pnpm --prefix tools/mission-driver test` green; `node tools/mission-driver/src/mission-check.mjs missions/<name>.json .` validates an existing mission.
- [x] Owner-doc update: `tools/mission-driver/CONTEXT.md` "Mission 配置系统" 优先级 line + 关键约束 note the new `promptsDir` field and the 3-level resolution chain.
- [x] `docs/logs/` updated.
...
- [x] in-scope behavior is complete (mission.promptsDir honored by main flow + subflows; fallback chain intact)
- [x] relevant docs aligned (CONTEXT.md updated)
- [x] verification has run: `pnpm --prefix tools/mission-driver test` + `node tools/mission-driver/src/mission-check.mjs missions/<name>.json .`
- [x] scoped verification is not conflated with full verification (full test suite + a real mission-check run — no scope limitation)
...
Status Note: Completed. Phase 1 (single phase, config-plumbing across 4 files) implemented: config.js resolves `missionPromptsDir` (abs or ""); main.js builds `[missionPromptsDir, shared].filter(Boolean)`; flow-loader.js loadSubFlow mirrors the chain with falsy-missionsDir guard preserved; mission-check.mjs existence-validates promptsDir (optional field). Resolution chain `mission.promptsDir → missions/prompts/ → TOOL_ROOT/prompts/` applies uniformly to main flow + subflows. 11 new tests in test/prompts-dir.test.js cover all three levels + backward compat + mission-check reject/accept/optional. Full verification green: 593 pass/0 fail, web typecheck+build OK, lint:prompts OK, mission-check.mjs validates an existing mission. CONTEXT.md owner-doc updated. Log entry recorded in docs/logs/2026/07-29.md. Solo cold-replay review per Reviewer-Availability Fallback (non-protected, non-high-risk: no API/data/auth/contract change, no flow-JSON edit, zero deps).
