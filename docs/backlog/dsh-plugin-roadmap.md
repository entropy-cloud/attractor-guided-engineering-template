...
> Last Updated: 2026-08-23 (M1 WI1–WI5 全部 `done`；M2-WI6 `done`——plugin/dsh 脚手架 + manifest/patch + 构建打包闭包门禁落地，plan `2026-08-23-1447-1`；M2-WI7 `done`——NativeExecutor 派发链 + engine-bridge 选择工厂/native config 接线落地（fake-agents 单元域验证），plan `2026-08-23-1447-2`；M2-WI8 `done`——L2 双后端行为矩阵落地并接 CI merge-blocking 门禁（`verify-age.sh` + `age-ci.yml`），plan `2026-08-23-1447-3`；M2-WI9 `done`——L3 宿主集成骨架 `host-harness.mjs` 落地（R3 §6 宿主启动组合收口 + 16 包 pinned devDeps 组合 fixture + 手写 NDJSON 传输 + 四场景 runner + env 门禁 `verify:native`/keyless 双形态，keyless 绿跑记录 `docs/testing/2026/08-23.md`），plan `2026-08-23-1621-1`；WI10 `ready`——plan `2026-08-23-1621-2` 经独立 draft review（2 轮）共识激活；`mdcontrol.draft/analyze` 路由经 plan 内裁定显式归属 M3〔WI12 完成前置〕)
...
- WI10 mdcontrol.run 异步作业契约:启动即返回 {runId, status},引擎作为 detached 宿主任务继续;完成可选 followup 回执(先例 draft-job.mjs);native 形式端到端跑通 demo mission,run-state 形状与 CLI 一致: `ready`（plan `docs/plans/dsh-plugin/2026-08-23-1621-2-mdcontrol-run-async-contract-e2e.md` 独立 draft review 2 轮共识 `acceptable as-is` 后激活；`mdcontrol.draft/analyze` 路由经 plan 内裁定显式归属 M3〔WI12 完成前置〕）
