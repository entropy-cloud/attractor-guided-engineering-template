...
- [x] WI13 routing-core.ts 编排层(decide(classify, retry, model)→ 4 类 RoutingDecision)。(plan `docs/plans/multi-plugin-dsh/2026-08-28-1312-2` Phase 1：pure `decide(request, config, now=0): RoutingDecision`——编排只做策略合成与优先级：partial:marker 最先 → Transform〔提取未闭合 `<AI_STEP_RESULT>` payload 成结构化 transform 对象〕→ retryDecision 重试预算 → pickModel 历史感知选模；Retry〔同模型退避，retry-after 优先于曲线〕/ Fallback〔failed 模型被历史 taint 或预算耗尽但有链内替代 → 换模型〕/ Transform / Give-up〔permanent:*与 unknown 原样返回、预算耗尽且无替代〕四类；三纯模块各自被独立调用零判别重复；零状态/零墙钟〔时间只走 `now` 入参〕/零随机；`test/routing-core.test.mjs` **16 例**全绿〔4 决策类各 ≥2 + 组合边界：maxRetries 走向分界 / permanent Transform-Give-up 分界 / transient × 链在库-耗尽两分支 / 同输入双跑 bit-identical〕；确定性 grep〔Date.now/Math.random/new Date/process.env/readFileSync/writeFileSync〕src/routing-core.ts 零命中)

- [x] WI14 noproute-routes.ts 路由层 + HTTP dispatcher(noproute.route/classify/pick-model/health 四方法 + /noproute/api/* 接线)。(plan `2026-08-28-1312-2` Phase 2：`createNopRouteRoutes({config, histogram?, logger?})` 返回 wire-method 全名 record 四 handler **全 sync**〔route=decide / classify=classify / pick-model=pickModel / health=版本+fallback 链+直方图〕+ `registerNopRouteHttpDispatcher`〔`ctx.get('webServer')` 缺席=降级 log 行非失败；在场=`/noproute/api/<method>` prefix 注册，mdcontrol 同形 + `{ok:true,value}`/`{ok:false,error:{code,message}}` envelope + 405/404/400/500 面〕；参数校验结构化 NopRouteError〔缺 error key / attempt 非负整数 / history 条目形状 / effort 枚举 / tokens 正数〕；health 直方图 = 注入的 service 层状态对象〔record/snapshot 稳定排序/reset 返回复位前快照〕；config 无 defaultModel 挂载期 fail-fast；`test/noproute-routes.test.mjs` **17 例**全绿；`./verify-age.sh` L1+L2+L2.5 GREEN〔990+423+88+18+119〕+ `git diff --stat tools/mission-driver/` 空〔引擎零 diff 自证〕)

- [x] WI15 service.ts 挂载(noproute service publication + HTTP dispatcher + headless 降级 + mount log)。(plan `2026-08-28-1312-2` Phase 3：`apply(ctx, config)` 发布 cordis `Service` 子类 **`noproute`**〔去 `nop-` 前缀 camelCase，设计 §Naming Convention〕挂 wire record + service 层直方图〔route/classify 累加、health 读取、`resetHistogram()` 复位语义随 service〕+ `ctx.inject(['webServer'], …)` 可选 HTTP 面〔缺席 = headless 降级 mount-log 行非挂载失败〕+ mount log 落 `nop-route` 包名与 `nopRoute` realm；不 inject `agents`（零 dispatch 纪律）；`test/service.test.mjs` **9 例**全绿〔stub ctx 直调 apply：发布形状/mount log/headless 降级+inject 挂点/直方图三面/零 agents inject〕；**M3 Deferred 收编**——manifest 增 nop-route 条目〔path `./nop-route`/realm `nopRoute`/config 镜像 bundle patch 三元组，分期注记更新为全在库 bundle〕；**真宿主四腿**〔scratch profile `nop-route-audit`，dump 面用 M3 as-built 合法 CLI 形 `dsh --profile <p> --dump-config`〕：① dry-run 双 add 计划零执行 ② 双 realm dump 并存〔`# == nop-age` `isolate: { nopAge: true }` + `# == nop-route` `isolate: { nopRoute: true }` / `id: nop-route-service` / config 三元组零漂移〕③ 二次执行全 already-present ④ unmount-all→重挂 dump `diff` 为空 + profile 清理〔list 空 + 目录删除，`web` profile 未触碰〕；**M2 Deferred 半边收编**——packaging/integration/dev-guide 三 owner docs 各增 §nop-route Plugin as-built 段〔完整两 bundle 结构性改写注记归 M5-WI18〕；`npm --prefix plugin/nop-route test` **97/97** + `./verify-age.sh` 全门 GREEN〔引擎 990+nop-age 423+nop-route 97+launcher 18+真值表 119〕+ 引擎树零 diff)
