...
- **[P1-followup] Manuals CHECK semantics doc-drift sweep (ASCII diagrams + "first loop" prose + one-sentence summary) — RESOLVED 2026-07-30.** The [P1] plan above corrected the manuals' step-table rows and `commands` examples but explicitly moved the two highest-visibility surfaces out of scope: the "30-second version" ASCII loop-diagram captions (`en.md:39` / `zh.md:39`) and the "first loop" prose walkthrough bullets (`en.md:341` / `zh.md:547`) still carried the stale "health check: do tests/build pass?" framing that contradicts shipped mdr-fix-3 behavior, plus the one-sentence summary stage label (`en.md:11` / `zh.md:11`) read `health-check` / `健康检查`. This sweep corrects all 6 drift locations to deterministic-state-gate semantics, and adjudicates the summary label identically across both manuals (Decision outcome A: aligned to neutral `state-check` / `状态检查`). Resolved by plan `docs/plans/mission-driver-actionable-fixes/2026-07-30-0936-1-check-semantics-doc-sweep.md` (executed + closed; docs-only — no flow JSON / engine / prompts / web-dist / dependency change; `pnpm --prefix tools/mission-driver test` → 597 pass / 0 fail, `prompt-check: OK`, web build produces no `web/dist` diff; independent closure audit `ses_04f4653b7ffe7ijqBE6u0o2tKo` PASS).
  - Source: deferred follow-up recorded in the [P1] plan's Closure out-of-scope note + Follow-up; same drift class as the [P1] finding above.
