...
> **L4 implementation note (landed 2026-08-23, M2-WI10)** — the dual-form leg is implemented at `plugin/dsh/scripts/e2e-demo.mjs` (+ shared scripted-model policy `scripts/e2e-policy.mjs`, composition fixture `test/fixtures/e2e.cordis.yml`): CLI leg = the real standalone engine child process over a PATH-first `opencode` stub; native leg = a real cordis runtime booted in-process (`@deepseek-ai/dsh-app-boot` `boot()`, 17th exact-pinned devDep) with the `mdcontrol.run` async contract exercised on the real route surface (`ctx.get('mdcontrol')`, route direct calls — the in-session `mission-control-run` skill entry joins at M3-WI12 and reuses this composition). Gate: `npm --prefix plugin/dsh run verify:e2e` (explicit local invocation, deterministic scripted model — keyless/stub-only by design, never CI-blocking, R3 §5 posture). Recorded green runs (×3, incl. normalized dual-leg run-state diff EMPTY, per-step markers-parsed assertions, one artificial correction-retry observed and recovered, monitor render check): `docs/testing/2026/08-23.md`; the same note records the monitor step-log `oc-`-prefix boundary finding (`docs/bugs/2026-08-23-monitor-native-log-naming.md`).
> **L4 onboarding extension (landed 2026-08-23, M3-WI11, plan `2026-08-23-1852-1`)** — the same `verify:e2e` entry now drives a SECOND dual-form pair over the `onboarding` mission on the real `mission-driver` flow (install-shape fixture replica verbatim from `template/install/missions/`; bounded 3-turn deterministic script; mechanism-plane assertions: markers valid for the real flow transitions + normalized run-state diff EMPTY — doc-quality semantics deliberately outside the deterministic gate, recorded "verification scope limited"), plus machine-pinned monitor render checks over all four runs (stepLogs/`/logs/:step`/node-detail, dual `oc-`/`native-` labels — the WI10 finding fixed) and descriptor-health assertions over the native legs' persisted child session logs. Evidence: `docs/testing/2026/08-23.md` (WI11 note).
...
> **L4 gate landed (2026-08-23, M2-WI10)** — `npm --prefix plugin/dsh run verify:e2e` (`scripts/e2e-demo.mjs`): the run-state diff output promised above is produced as `<scratch>/e2e-report.json` (raw + normalized per-leg run-states, stub request log, per-leg assertions). Explicit local invocation only — deterministic scripted model domain, zero credentials, never wired into `./verify-age.sh` / `age-ci.yml` (verified: the aggregate gate stays green without any env). Recorded green runs: `docs/testing/2026/08-23.md`.
