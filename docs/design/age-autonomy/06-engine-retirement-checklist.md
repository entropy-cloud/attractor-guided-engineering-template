...
| 3 | 审计预算：`engine.js` maxAuditRounds 轮门（DEEP_AUDIT 轮计数与拒新轮） | law `audit-rounds-overflow` 门禁（roadmap DAR 新 dispatch 行时预算耗尽 → deny）+ terminal-rules R1 预算硬门（收口 partial/blocked，不得静默 completed）+ 恢复重派同 occurrence 零自增（豁免面防死锁）——双面一个预算（1411-3 成文） | `src/law-rules.mjs`、`plugin/dsh/src/supervisor/terminal-rules.ts`；`plugin/dsh/test/{law-truth-table,supervisor-terminal}.test.mjs`；plan 2026-08-25-0815-3 / 2026-08-26-1411-3 / 2026-08-26-1954-2 | 无 | 判定：**已覆盖** |
...
| 9 | 终态与退出码面：`src/exit-map.js` EXIT_MAP 现行 11 键冻结契约（+ EXECUTION-PRINCIPLE §11 逐行同步纪律）；`skipped`/动态 done 词有意不映射 | DSH 形态走回执不依赖退出码（A8 尽力投递 + `mdcontrol.status` 透出 partial/blocked）；独立形态 `partial`/`blocked` 增补 = M5-WI38 独立立项（plan 2026-08-27-1023-2 在库 active——本行按「**增补中**」如实登记，不阻塞判定门） | `src/exit-map.js`；`test/exit-map.test.js`；plan 2026-08-27-1023-2（增补中） | WI38 未收口（增补中；终态映射纪律 03 §8 成文：独立立项修改冻结契约） | 判定：**引擎留任面**（冻结契约，存续期不变；增补走 WI38） |
...
**结论：引擎留任主后端（条件退役）。** 门禁 + 守夜人已覆盖**账本域全部职责**（审计预算/失败熔断/崩溃恢复/连续队列/终态规则/评审审计派发——矩阵 #3/#4/#5/#7/#8 判定腿）；但引擎仍独占承载：run 内执行语义面（transient 退避/max* 循环防护/step 重试——#1/#2/#4 run 内腿）、execute 腿编排（#6/#8）、run-state/monitor 域（#5 引擎侧/#10）、冻结契约（#9 EXIT_MAP）与双读协议（#11 marker）。00 §3「引擎定位」成文沿袭：初期主后端、可替换实现——**「可替换」的证据面已齐（本矩阵），替换的执行面未开始（退役执行期）**。
...
| G3 | EXIT_MAP `partial`/`blocked` 显式增补（独立形态终态退出码——03 §8 终态映射纪律） | M5-WI38 独立立项（plan 2026-08-27-1023-2 增补中——冻结契约变更 + `exit-map.test.js`） | 本组（1023-2，进行中） |
