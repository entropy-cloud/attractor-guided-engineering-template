...
| 8 | 主流程编排与 execute 腿：`flows/mission-driver.json` 五步循环 CHECK→REVIEW_PLANS→EXECUTE_PLANS→DRAFT_PLANS→DEEP_AUDIT + deep-audit-loop | 队列/评审/审计腿 = trigger-eval 14 谓词 + 连续模式（roadmap 即队列，opt-in 门 + 终态链式沿 + 心跳兜底）+ nothing→deep-audit 派发（预算计量同写；**判据已扩 roadmap 未勾维度**——law `nothing-claim-guard` 信号携带 `roadmapUnchecked: N` + deep-audit 派发为强制沿，M5-WI48/plan 2026-08-27-2122-2 交付）+ R1–R4 终态收口 + run-terminal 回执 | `plugin/dsh/src/supervisor/{trigger-eval,decision-core,watchdog,terminal-rules,exec-arm}.ts`、`src/law-rules.mjs`；`plugin/dsh/test/{supervisor-trigger,supervisor-continuous,law-truth-table}.test.mjs`（43/15/119 例）；plan 2026-08-26-1411-2 / 1954-1 / 2026-08-27-2122-2 | execute 腿 = 引擎 run（→ D4）；无引擎 run 宿主的停滞由 R3/R4 捕获 blocked + 回执（诚实失败面，非缺口） | 判定：**引擎留任面**（execute 腿）；队列/评审/审计腿已覆盖（nothing→deep-audit 未勾维度 law 面已扩，2122-2 注记） |
| 9 | 终态与退出码面：`src/exit-map.js` EXIT_MAP 现行 13 键冻结契约（M5-WI38 已增补 `partial`/`blocked` → exit 3；+ EXECUTION-PRINCIPLE §11 逐行同步纪律）；`skipped`/动态 done 词有意不映射 | DSH 形态走回执不依赖退出码（A8 尽力投递 + `mdcontrol.status` 透出 partial/blocked）；独立形态 `partial`/`blocked` 增补 = M5-WI38 独立立项**已收口**（plan 2026-08-27-1023-2：EXIT_MAP 11→13 键 + `exit-map.test.js` 13→19 例钉住 + EXECUTION-PRINCIPLE §11 同步，2026-08-27）；守夜人终态真值 = terminal-rules R1–R4（R1 未勾 → partial、不得静默 completed）+ law `nothing-claim-guard` 未勾维度（M5-WI48 扩面） | `src/exit-map.js`；`test/exit-map.test.js`；`plugin/dsh/src/supervisor/terminal-rules.ts`；`plugin/dsh/test/law-truth-table.test.mjs`；plan 2026-08-27-1023-2（已收口）/ 2026-08-27-2122-2 | **G8**：引擎形态终态真值不读全局未勾——`_shouldCompleteOnAuditQuota` 清洁短路（plans-only 判据，`openAudits()` 自 M2-WI22 退役恒空）与 maxAuditRounds 轮门终态直接发射 `completed`（engine.js:728-749/:1668-1673），未勾发现 × DRAFT_PLANS nothing → 静默 completed/exit 0（R6 实证；03 §8 R1 as-built 豁免注记在案） | 判定：**部分覆盖**（守夜人终态真值 + EXIT_MAP 增补面已覆盖/已收口；引擎形态终态真值缺口 = retirement-gated **G8**，M5-WI48；EXIT_MAP 冻结契约本体 = 引擎留任面） |
...
| G8 | 引擎形态终态真值不读全局未勾（`_shouldCompleteOnAuditQuota` 清洁短路 plans-only 判据 + maxAuditRounds 轮门终态 direct `completed`——未勾发现 × DRAFT_PLANS nothing → 静默 completed/exit 0，R6 实证，M5-WI48 立案；law `nothing-claim-guard` 未勾维度扩面 + 03 §8 R1 as-built 豁免已随 2122-2 成文，引擎缺口本身仍在） | 引擎读全局账本归退役执行期 / 独立形态后端替代立项（G1/G2/G7 同族；route ① 清洁短路/轮门终态前增读 roadmap 未勾因零引擎 diff 底线否决——2122-2 Phase 1 Decision） | 退役执行期 / 后继 mission |
