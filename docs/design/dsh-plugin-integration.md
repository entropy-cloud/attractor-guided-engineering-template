...
> **Status: SUPPORTED BASELINE (delivered 2026-08-23 — P1–P4 all closed; phase records in `docs/architecture/dsh-plugin-packaging.md` §Phased Delivery).** Both consumption forms are live: the standalone CLI (default) and the DSH plugin form. As-built notes are inline throughout (dated M2-WI10/M3-WI12/M4-WI14 markers below); this header previously carried the pre-landing "PLANNED" caveat and was corrected 2026-08-28 during the multi-plugin consistency pass. 2026-08-28 final state (M5-WI18): the plugin form is the **two-bundle `nop-*` family** (`nop-age` + `nop-route`); this doc family is the `nop-age` feature design (Mission Control) plus the family-level product faces (§Multi-Plugin Family As-Built) — the earlier「migration note + nop-route increment」juxtaposition was structurally rewritten at M5-WI18 (plan `docs/plans/multi-plugin-dsh/2026-08-28-1540-2`).

## Multi-Plugin Family As-Built (nop-age + nop-route, final-state rewrite M5-WI18)

The multi-plugin refactor (mission `multi-plugin-dsh`, design owner `docs/design/multi-plugin-dsh-architecture.md`, all five milestones closed 2026-08-28) repackaged the single DSH bundle as a **two-bundle plugin family**. This doc is the **feature design of the `nop-age` plugin** (Mission Control — everything from §Purpose down); this section carries the family-level product faces. The `nop-route` product surface follows below.

- **Family shape**: `plugin/nop-age/` (package `nop-age`, isolate realm `nopAge`, cordis services `mdcontrol` + `mdsupervisor`, HTTP prefix `/mdcontrol/api`, three `mission-control-*` skills, `preset/age`) and `plugin/nop-route/` (package `nop-route`, isolate realm `nopRoute`, cordis service `noproute`, HTTP prefix `/noproute/api`, no skills/preset — programmatic-only). Service names follow the bundle-minus-`nop-`-prefix camelCase rule; `mdcontrol` is the legacy exception kept for API stability (token-map carve-out, design §Naming Convention).
- **Install face**: one `./plugin/load-plugins.sh` run mounts both bundles from `plugin/plugin-manifest.yml` (dual-entry end state since M4-WI15; 7 flags; idempotent; `${PROJECT_ROOT}` must be exported — the nop-age `supervisor.projectRoot` placeholder). Manual per-bundle `dsh plugin --profile <p> add link:...` remains the fallback (dev guide §Setup). Joint-mount evidence (both realms coexist in one dump, `mdcontrol` structurally unique, idempotent, unmount-remount end-state identity): M5-WI17 six legs, plan `docs/plans/multi-plugin-dsh/2026-08-28-1540-1`.
- **Verification posture for consumers**: the aggregate gate `./verify-age.sh` (L1 engine + L2 both plugin suites + launcher stubs + L2.5 law gates) is the merge/CI face; the e2e legs (`nop-route` `verify:e2e` environment-independent; `nop-age` `verify:e2e` self-contained stub model endpoint; `nop-age` `verify:e2e:preset`) are explicit local gates outside CI — family map with counts in `docs/architecture/dsh-plugin-packaging.md` §Multi-Plugin Family As-Built.
- **Known boundaries**: neither bundle exposes a package entry (`main`/`exports`) — real-host boot import is an independent successor item (M2-WI4 record); cross-plugin composition (`nop-route` consuming `mdcontrol` results) stays an explicit Non-Goal with recorded reopen triggers (design §Deferred But Adjudicated).

## nop-route Plugin (as-built product surface, M4 landed 2026-08-28)
