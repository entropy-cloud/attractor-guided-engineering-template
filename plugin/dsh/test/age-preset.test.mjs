 ...
 *
 *   D2 — shape: `plugin/dsh/preset/age/` holds agent.cordis.yml +
 ...
 *        (`dsh-mission-control`; the service stays mounted exactly once by
...
test('D3: zero service rows — no dsh-mission-control mount face in the preset', () => {
  const rows = parseComposition(readFileSync(join(PRESET_DIR, 'agent.cordis.yml'), 'utf8'))
  for (const { row } of flattenRows(rows)) {
    assert.notEqual(row.name, pkg.name, `preset row must not re-mount the plugin service ("${pkg.name}" is bundle-patch-only)`)
    assert.ok(!String(row.name).includes('mission-control'), 'no mission-control service row')
  }
